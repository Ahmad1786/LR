...
def apply_LR(random_points, classification):
    big_X = []
...
for _ in range(num_experiments):
    ...
    indices_to_flip = np.random.choice(len(classification), num_to_flip, replace=False)
    for idx in indices_to_flip:
        classification[idx] = -1 if classification[idx] == 1 else 1

    weight_vector = apply_LR(random_points, classification)
